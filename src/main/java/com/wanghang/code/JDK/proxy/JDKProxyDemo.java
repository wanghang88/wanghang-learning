 ...
 *
 *2:如何实现JDK的动态代理：
 * 1)创建一个实现接口InvocationHandler的类，它必须实现invoke方法(本案例中的InvocationHandlerImpl,它又一个成员变需要代理的接口)
 * 2)创建被代理的类以及接口(本案例中的IUservice接口)
 * 3)通过Proxy的静态方法,newProxyInstance(ClassLoaderloader, Class[] interfaces, InvocationHandler h),
